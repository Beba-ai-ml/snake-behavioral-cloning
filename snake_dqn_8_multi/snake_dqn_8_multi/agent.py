...
class Agent:
    ...
    def act_batch(self, states: List[np.ndarray]) -> List[int]:
        if not states:
            return []
        ...
        return torch.argmax(q_values, dim=1).cpu().tolist()
    ...
    def update_epsilon(self) -> None:
        # NoisyNet handles exploration; epsilon is kept constant at minimum.
